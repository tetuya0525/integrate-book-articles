...
class ArticleIntegrator:
    # (ロジックはv2.0と同様)
    def __init__(self, db: FirestoreClientProtocol, config: Config):
        self.db = db
        self.config = config
    ...
    @backoff.on_exception(
        ...
        on_backoff=lambda details: log_structured(
            "WARNING",
            f"リトライします (試行 {details['tries']}回目)",
            document_id=details["args"][0] if details.get("args") else "unknown",
            error=details["exception"],
        ),
    )
    def integrate(self, doc_id: str) -> bool:
        staging_ref = self.db.collection(self.config.STAGING_COLLECTION).document(
            doc_id
        )
...
def log_structured(level: str, message: str, **kwargs: Any) -> None:
    ...
    logger_func(
        json.dumps(log_data, ensure_ascii=False, default=str), exc_info=exc_info
    )
...
def create_app() -> Flask:
    ...
    def handle_pubsub_message():
        ...
        try:
            ...
            if success:
                log_structured(
                    "INFO", "ドキュメントの統合に成功しました", document_id=doc_id
                )
                return "Success", 204
            else:
                log_structured(
                    "INFO",
                    "ドキュメントは既に統合済みのためスキップしました",
                    document_id=doc_id,
                )
                return "Skipped", 200
    ...
    def get_status():
        try:
            ...
            staging_count = (
                db_client.collection(config.STAGING_COLLECTION)
                .count()
                .get()[0][0]
                .value
            )
            final_count = (
                db_client.collection(config.FINAL_COLLECTION).count().get()[0][0].value
            )
            return jsonify(
                {
                    "status": "ok",
                    "staging_documents": staging_count,
